...
def nest_sentences(document, l, tokenizer):
  nested = []
  sent = []
  length = 0
  for sentence in nltk.sent_tokenize(str(document)):
    length += len(tokenizer.tokenize(sentence))

    if length < l:
      sentence = ' [CLS] [SEP] ' + sentence
...
def load_text(args, text, target_fp, device):
    ...
    nested = nest_sentences(text, 512, tokenizer)
    for n in nested:
        x = ' '.join(n)
